...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Repair Order Request List</title>
        <script src="https://cdn.tailwindcss.com"></script>
    </head>
    <body class="bg-gray-100 font-sans">

        <div class="max-w-7xl mx-auto p-6">
            <h1 class="text-3xl font-bold text-blue-800 mb-6 border-b pb-2">Repair Order Request List</h1>

            <!-- FILTER FORM -->
            <form method="get" action="repair-request-list" class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6 bg-white p-4 rounded shadow">
                <div>
                ...
                <table class="min-w-full table-auto text-sm text-left text-gray-700">
                    <thead class="bg-blue-600 text-white">
                        <tr>
                            ...
                            <th class="px-4 py-3">Estimated Price</th>
                    ...
                    <tbody>
                        <c:forEach var="ro" items="${roList}" varStatus="i">
                            <tr class="border-t hover:bg-gray-50 transition">
                                <td class="px-4 py-2">${(currentPage - 1) * pageSize + i.index + 1}</td>
                                <td class="px-4 py-2">${ro.createdByName}</td>
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2 text-left">
                                    <c:choose>
                                        <c:when test="${not empty ro.details}">
                                            <fmt:formatNumber value="${ro.details[0].unitPrice}" type="number" groupingUsed="true" maxFractionDigits="0" /> VND
                                        </c:when>
                                        <c:otherwise>-</c:otherwise>
                                    </c:choose>
                                </td>

                                <!-- Status with RO Created logic -->
                                <td class="px-4 py-2">
                                    <c:choose>
                                        <c:when test="${ro.status == 'Approved'}">
                                ...
                                <td class="px-4 py-2">${ro.note}</td>
                                <td class="px-4 py-2">
                                    <a href="${pageContext.request.contextPath}/repair-order-detail?id=${ro.roId}"
            ...
            <div class="mt-4 flex justify-center gap-2">
                <c:if test="${currentPage > 1}">
                    <a href="repair-request-list?page=1&status=${filterStatus}&createdDate=${filterDate}&createdByName=${filterCreatedByName}"
                       class="px-3 py-1 border rounded bg-white text-gray-700 hover:bg-gray-100">&laquo;</a>
                    <a href="repair-request-list?page=${currentPage - 1}&status=${filterStatus}&createdDate=${filterDate}&createdByName=${filterCreatedByName}"
                       class="px-3 py-1 border rounded bg-white text-gray-700 hover:bg-gray-100">&lt;</a>
                </c:if>

                <c:forEach begin="1" end="${totalPages}" var="i">
                    <a href="repair-request-list?page=${i}&status=${filterStatus}&createdDate=${filterDate}&createdByName=${filterCreatedByName}"
                ...
                <c:if test="${currentPage < totalPages}">
                    <a href="repair-request-list?page=${currentPage + 1}&status=${filterStatus}&createdDate=${filterDate}&createdByName=${filterCreatedByName}"
                       class="px-3 py-1 border rounded bg-white text-gray-700 hover:bg-gray-100">&gt;</a>
                    <a href="repair-request-list?page=${totalPages}&status=${filterStatus}&createdDate=${filterDate}&createdByName=${filterCreatedByName}"
