...
public class RequestUpdateServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            List<RequestDetailItem> requestItems = getRequestDetails(Integer.parseInt(requestId));
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (requestId == null || materialIds == null || actualQuantities == null) {
            response.sendRedirect("error.jsp");
            return;
        }
        
        try {
            boolean success = updateRequestActualQuantities(Integer.parseInt(requestId), 
                    materialIds, actualQuantities);
...
try (Connection conn = db.getConnection();
    ...
    private boolean updateRequestActualQuantities(int requestId, String[] materialIds, 
            String[] actualQuantities) throws SQLException {
        
        String sql = "UPDATE RequestDetail SET ActualQuantity = ? " +
                    "WHERE RequestId = ? AND MaterialId = ?";
...
try (Connection conn = db.getConnection();
            ...
            for (int i = 0; i < materialIds.length; i++) {
                pstmt.setInt(1, Integer.parseInt(actualQuantities[i]));
                pstmt.setInt(2, requestId);
                pstmt.setInt(3, Integer.parseInt(materialIds[i]));
                pstmt.addBatch();
            }
