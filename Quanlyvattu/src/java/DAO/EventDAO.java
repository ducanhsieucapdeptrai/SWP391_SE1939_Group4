...
public class EventDAO extends DBContext {
    ...
    public List<Event> getEventsByUserId(int userId, Timestamp start, Timestamp end) {
        ...
        // This query finds events that overlap with the given time range [start, end].
        // An event overlaps if it starts before the range ends AND it ends after the range starts.
        String sql = "SELECT * FROM Events WHERE UserId = ? AND StartTime < ? AND EndTime > ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, userId);
            ps.setTimestamp(2, end); // Event must start before the view ends
            ps.setTimestamp(3, start); // Event must end after the view starts
